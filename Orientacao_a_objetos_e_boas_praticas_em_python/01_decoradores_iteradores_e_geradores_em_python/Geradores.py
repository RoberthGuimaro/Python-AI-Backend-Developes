"""
    O que são geradores?
        ...
        - São tipos especiais de iteradores, ao contrário das listas
        ou outro iteráveis, não armazenam todos os seus valores na
        memória.

        - São definidos usando funções regulares, mas, ao invés de
        retornar valores usando "return", utilizam "yield".

    Características de geradores
        - Uma vez que um item gerado é consumido, ele é esquecido
        e não pode ser acessado novamente.

        - O estado interno de um gerador é mantido entre chamadas.

        - A execução de um gerador é pausada na declaração "yield"
        e retomada daí na próxima vez que ele for chamado.
    ...
    Recuperando dados de uma API
        ...
        - Quando todos os produtos de uma página forem
        retornador, verificar se existem novas páginas.
